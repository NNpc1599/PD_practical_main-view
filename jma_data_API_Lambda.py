...
def Data_getter(area_code):
    ...
    chrome_options.binary_location = "/opt/bin/headless-chromium" #Lambda Layerに配置されたHeadless Chromiumのパス
    chrome_options.add_argument("--headless")  # ヘッドレスモー
    chrome_options.add_argument("--no-sandbox")
    chrome_options.add_argument("--disable-dev-shm-usage") # 共有メモリ使用を無効化

    # ChromeDriverのインスタンスを作成
    service = Service("/opt/bin/chromedriver")  # Lambda Layerに配置されたChromeDriverのパス
